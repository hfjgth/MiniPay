...
class PaymentServiceTest {
    ...
    void testProcessPay_AmountMatch() {
        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_001"))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess("{\"code\":0,\"data\":{\"amount\":99.99}}", MediaType.APPLICATION_JSON));

        // 状态更新请求，允许成功或失败（模拟随机）
        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_001/status"))
    ...
    void testProcessPay_AmountMismatch() {
        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_002"))
                .andExpect(method(HttpMethod.GET))
    ...
    void testProcessPay_OrderServiceDown() {
        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_003"))
                .andRespond(withServerError());
        // GET 失败后仍会尝试 PUT 同步状态，PUT 也失败则进内存
        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_003/status"))
                .andExpect(method(HttpMethod.PUT))
                .andRespond(withServerError());
    ...
    void testProcessPay_StatusUpdateFail() {
        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_004"))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess("{\"code\":0,\"data\":{\"amount\":10.00}}", MediaType.APPLICATION_JSON));

        mockServer.expect(requestTo("http://localhost:8081/api/v1/orders/ORDER_004/status"))
                .andExpect(method(HttpMethod.PUT))
                .andRespond(withServerError());
